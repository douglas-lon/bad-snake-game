...
class Snake:
    ...
    def add_part(self):
        ...
        if self.snake[-1].x > self.last_pos[-1][0]:
            coord[0] = self.snake[-1].x - self.distance
            coord[1] = self.snake[-1].y
            last_coord = [coord[0] - self.distance, coord[1]]
        elif self.snake[-1].x < self.last_pos[-1][0]:
            coord[0] = self.snake[-1].x + self.distance
            coord[1] = self.snake[-1].y
            last_coord = [coord[0] + self.distance, coord[1]]
        elif self.snake[-1].y > self.last_pos[-1][1]:
            coord[0] = self.snake[-1].x
            coord[1] = self.snake[-1].y - self.distance
            last_coord = [coord[0], coord[1] - self.distance]
        elif self.snake[-1].y < self.last_pos[-1][1]:
            coord[0] = self.snake[-1].x
            coord[1] = self.snake[-1].y + self.distance
            last_coord = [coord[0] , coord[1] + self.distance]
            
        self.snake.append(pygame.Rect(coord[0], coord[1], 32,32))
        self.last_pos.append(last_coord)
    ...
    def snake_events(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            if self.travelled >= self.distance + self.vel:
                ...
                if event.key == pygame.K_LEFT and (self.direction != Direction.RIGHT and self.direction != None):
